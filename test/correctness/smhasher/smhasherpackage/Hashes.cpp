...
extern "C" {
  ...
  uint64_t vhash4smhasher(const void *key, int len);
}
  
uint32_t saved_seed=0; // 32 and 64 shared
void CLHASH32 ( const void * key, int len, uint32_t seed, void * out ) {
  if (saved_seed != seed) {
    init_cl3264(seed);
    saved_seed = seed;
  } 
...
uint32_t saved_seed_vhash=0;
void VHASH64 ( const void * key, int len, uint32_t seed, void * out )
{
  if (saved_seed_vhash != seed) {
    init_vhash(seed);
    saved_seed_vhash = seed;
  }
  *(uint64_t*)out = vhash4smhasher(key,len);
}
