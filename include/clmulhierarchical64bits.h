...
uint64_t CLHASHtmp(const void* rs, const uint64_t * string,
		const size_t length) {
	assert(sizeof(size_t)<=sizeof(uint64_t));// otherwise, we need to worry
	assert(((uintptr_t) rs & 15) == 0);// we expect cache line alignment for the keys
	const unsigned int m = 128;// we process the data in chunks of 16 cache lines
	...
	if (m < length) { // long strings
		...
		if (t + 4 * m <= length) {
			__m128i polyvalue2 = mul128by128to128_lazymod127(polyvalue,
					polyvalue);
			__m128i polyvalue3 = mul128by128to128_lazymod127(polyvalue,
					polyvalue2);
			__m128i polyvalue4 = mul128by128to128_lazymod127(polyvalue2,
					polyvalue2);

			for (; t + 4 * m <= length; t += 4*m) {
				const __m128i h1 = __clmulhalfscalarproductwithoutreduction(
						rs64, string + t, m);
				const __m128i h2 = __clmulhalfscalarproductwithoutreduction(
						rs64, string + t + m, m);
				const __m128i h3 = __clmulhalfscalarproductwithoutreduction(
						rs64, string + t + 2 * m, m);
				acc = mul128by128to128_lazymod127_4by4(polyvalue, polyvalue2,
						polyvalue3, polyvalue4, h3, h2, h1, acc);
				const __m128i h4 = __clmulhalfscalarproductwithoutreduction(
						rs64, string + t + 3 * m, m);

				acc = _mm_xor_si128(acc, h4);
			}
		}
