...
static __m128i __clmulhalfscalarproductwithoutreduction16(const __m128i * randomsource, const uint64_t * string) {
	//assert(((uintptr_t) randomsource & 15) == 0);// we expect cache line alignment for the keys
	// we expect length = 128, so we need  16 cache lines of keys and 16 cache lines of strings.
	//if(CLHASH_DEBUG) assert((length & 3) == 0); // if not, we need special handling (omitted)
	//const uint64_t * const endstring = string + length;
	...
	// we expect length = 128
	//for (; string + 3 < endstring; randomsource += 2, string += 4) {
	__m128i temp1, temp2, add1, clprod1,temp12,temp22,add12,clprod12;
        temp1 = _mm_load_si128( randomsource);
		temp2 = _mm_lddqu_si128((__m128i *) string);
		add1 = _mm_xor_si128(temp1, temp2);
		clprod1 = _mm_clmulepi64_si128(add1, add1, 0x10);
		acc = _mm_xor_si128(clprod1, acc);
		temp12 = _mm_load_si128(randomsource + 1);
		temp22 = _mm_lddqu_si128((__m128i *) (string + 2));
		add12 = _mm_xor_si128(temp12, temp22);
		clprod12 = _mm_clmulepi64_si128(add12, add12, 0x10);
		acc = _mm_xor_si128(clprod12, acc);

        temp1 = _mm_load_si128( randomsource + 2);
		temp2 = _mm_lddqu_si128((__m128i *) (string + 4));
		add1 = _mm_xor_si128(temp1, temp2);
		clprod1 = _mm_clmulepi64_si128(add1, add1, 0x10);
		acc = _mm_xor_si128(clprod1, acc);
		temp12 = _mm_load_si128(randomsource + 3);
		temp22 = _mm_lddqu_si128((__m128i *) (string + 6));
		add12 = _mm_xor_si128(temp12, temp22);
		clprod12 = _mm_clmulepi64_si128(add12, add12, 0x10);
		acc = _mm_xor_si128(clprod12, acc);

        temp1 = _mm_load_si128( randomsource + 4);
		temp2 = _mm_lddqu_si128((__m128i *) (string + 8));
		add1 = _mm_xor_si128(temp1, temp2);
		clprod1 = _mm_clmulepi64_si128(add1, add1, 0x10);
		acc = _mm_xor_si128(clprod1, acc);
		temp12 = _mm_load_si128(randomsource + 5);
		temp22 = _mm_lddqu_si128((__m128i *) (string + 10));
		add12 = _mm_xor_si128(temp12, temp22);
		clprod12 = _mm_clmulepi64_si128(add12, add12, 0x10);
		acc = _mm_xor_si128(clprod12, acc);

        temp1 = _mm_load_si128( randomsource + 6);
		temp2 = _mm_lddqu_si128((__m128i *) (string + 12));
		add1 = _mm_xor_si128(temp1, temp2);
		clprod1 = _mm_clmulepi64_si128(add1, add1, 0x10);
		acc = _mm_xor_si128(clprod1, acc);
		temp12 = _mm_load_si128(randomsource + 7);
		temp22 = _mm_lddqu_si128((__m128i *) (string + 14));
		add12 = _mm_xor_si128(temp12, temp22);
		clprod12 = _mm_clmulepi64_si128(add12, add12, 0x10);
		acc = _mm_xor_si128(clprod12, acc);
...
uint64_t CLHASH16(const void* rs, const uint64_t * string,
		const size_t length) {
	assert(sizeof(size_t)<=sizeof(uint64_t));// otherwise, we need to worry
	assert(((uintptr_t) rs & 15) == 0);// we expect cache line alignment for the keys
	const unsigned int m = 16;// we process the data in chunks of 16 cache lines
	if(CLHASH_DEBUG) assert((m  & 3) == 0); //m should be divisible by 4
	const int m128neededperblock = m / 2;// that is how many 128-bit words of random bits we use per block
	const __m128i * rs64 = (__m128i *) rs;
	__m128i polyvalue =  _mm_load_si128(rs64 + m128neededperblock); // to preserve alignment on cache lines for main loop, we pick random bits at the end
	...
	// we should check that polyvalue is non-zero, though this is best done outside the function and highly unlikely
	if (m < length) { // long strings
		__m128i  acc =  __clmulhalfscalarproductwithoutreduction16(rs64, string);
		size_t t = m;
		for (; t +  m <= length; t +=  m) {
			// we compute something like
			// acc+= polyvalue * acc + h1
