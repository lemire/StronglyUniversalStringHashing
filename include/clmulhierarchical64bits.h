...
// For use with hashCLMUL2Level
// we expect length to have value 128 or, at least, to be divisible by 4.
static __m128i __clmulhalfscalarproductwithoutreductionlong(const __m128i * randomsource, const uint64_t * string,
		const size_t length) {
	assert(((uintptr_t) randomsource & 15) == 0);// we expect cache line alignment for the keys
	// we expect length = 128, so we need  16 cache lines of keys and 16 cache lines of strings.
	assert((length & 7) == 0); // if not, we need special handling (omitted)
	//const uint64_t * const endstring = string + length;
	__m128i acc = _mm_setzero_si128();
	// we expect length = 128
	for(int k = 0; k<128; k+=8) {
//	for (; string + 3 < endstring; randomsource += 4, string += 8) {
          //IACA_START
		const __m128i temp1 = _mm_load_si128( randomsource);
		const __m128i temp2 = _mm_lddqu_si128((__m128i *) string);
		const __m128i add1 = _mm_xor_si128(temp1, temp2);
		...
		const __m128i clprod12 = _mm_clmulepi64_si128(add12, add12, 0x10);
		acc = _mm_xor_si128(clprod12, acc);
		const __m128i temp13 = _mm_load_si128( randomsource + 3);
		const __m128i temp23 = _mm_lddqu_si128((__m128i *) (string + 4));
		const __m128i add13 = _mm_xor_si128(temp13, temp23);
		const __m128i clprod13 = _mm_clmulepi64_si128(add13, add13, 0x10);
		acc = _mm_xor_si128(clprod13, acc);
		const __m128i temp14 = _mm_load_si128(randomsource + 4);
		const __m128i temp24 = _mm_lddqu_si128((__m128i *) (string + 6));
		const __m128i add14 = _mm_xor_si128(temp14, temp24);
		const __m128i clprod14 = _mm_clmulepi64_si128(add14, add14, 0x10);
		acc = _mm_xor_si128(clprod14, acc);
		randomsource += 4; string += 8;
...
uint64_t testme(const void* rs, const uint64_t * string,
		const size_t length) {
	if (length == 0)
		return 0; // hmmmm...
	assert(sizeof(size_t)<=sizeof(uint64_t));// otherwise, we need to worry
	...
	// we should check that polyvalue is non-zero, though this is best done outside the function and highly unlikely
	if (m < length) { // long strings
		__m128i  acc =  __clmulhalfscalarproductwithoutreductionlong(rs64, string,m);
		size_t t = m;
		for (; t +  m <= length; t +=  m) {
			// we compute something like
			// acc+= polyvalue * acc + h1
			acc =  mul128by128to128_lazymod127(polyvalue,acc);
			__m128i h1 =  __clmulhalfscalarproductwithoutreductionlong(rs64, string+t,m);
			acc = _mm_xor_si128(acc,h1);
		}
		int remain = length - t;
		if(remain > 0) {
